...
def train(train_data, model_name, save_model=False, metrics=False):
    ...
    # Define Features
    features = ['_Arterial', '_Collector', '_Local', '_Trail',
...
def predict_test(data, model='', model_fname='model_1.pkl'):
    """
    ...
    # Engineer features usign model functions.
    keep_rows = ['FEATURE_CODE_DESC','geometry', 'bikelane']
    ...
    # Rename columns, removeing the 'FEATURE_CODE_DESC' from the title
    
    
    print("[INFO] finished feature engineering")
    
    # Define Features - DONT USE TRAILS FOR NOW
    features = ['_Arterial', '_Collector', '_Local', '_Trail',
                #'x_region_2', 'x_region_3', 'y_region_2', 'y_region_3', 
                'bikelane',]
...
# Predicting results using the model.
def predict_general(data, model='', model_fname='model_1.pkl'):
    """
    Takes in a trained model (either just trained or from saved) and returns predictions + probabilities.
    ...
    #HARDCODING, MAKE A TRY EXCEPT:
    # If any of the features don't exist, we want to add that feature as a zero column
    X_test['_Trail'] = 0
